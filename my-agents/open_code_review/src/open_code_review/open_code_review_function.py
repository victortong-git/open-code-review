...
async def open_code_review_function(
    ...
    async def _response_fn(input_message: str) -> str:
        ...
        # Check if this is a QA consultant request
        if finding_id and review_type == "qa_consultant":
            # Handle QA consultant logic inline
            logger.info(f"Processing QA consultant request for finding ID: {finding_id}")
            
            # Get finding details using the helper function
            ...
            qa_prompt_file = os.path.join(current_dir, "prompts", "qa_consultant_review.txt")
            
            try:
                with open(qa_prompt_file, 'r') as f:
                    qa_prompt_content = f.read()
            except FileNotFoundError:
                error_msg = f"QA consultant prompt file '{qa_prompt_file}' not found"
                logger.error(error_msg)
                return json.dumps({
...
Code Content: {finding.get('code_content', 'No code content')}
            ...
            qa_ai_prompt_message = f"""QA Security Consultant Review Request:
...
"""
        ...
        if not file_id:
            error_msg = f"Invalid input. Please provide file_id or finding_id with review_type. Received: {input_message}"
            logger.error(error_msg)
            return json.dumps({
