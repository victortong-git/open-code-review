...
/**
 * Client for interacting with the NeMo Agent Toolkit (NAT) API
 */
export class NATClient {
  ...
  constructor() {
    this.baseUrl = config.nemoAgentToolkit.baseUrl;
    ...
    this.aiCodeAnalysisUrl = 'http://nvidia-nat:8000/generate'; // Direct service-to-service communication (no CORS issue)
  }
  ...
  async assessFile(fileContent: string, fileName: string, fileId: number): Promise<string> {
    try {
      ...
      try {
        ...
        console.error('NeMo Agent Toolkit server is not available. Make sure it is running:', error);
        throw new Error('NeMo Agent Toolkit server is not available. Please check if the service is running.');
      }
  ...
  /**
   * Subscribe to events from the NAT client
  ...
  /**
   * Send a request directly to the NeMo Agent Toolkit for AI code analysis
  ...
  async callAICodeAnalysis(fileId: number, reviewType: string): Promise<any> {
    try {
      const natUrl = 'http://nvidia-nat:8000/generate';
      console.log(`Calling NeMo Agent Toolkit directly at ${natUrl} for file ID: ${fileId}, review type: ${reviewType}`);

      const response = await axios.post(natUrl, {
      ...
      console.log('NeMo Agent Toolkit response:', response.data);
      ...
      console.error('Error calling NeMo Agent Toolkit:', error.message);
      if (error.response) {
        console.error('NeMo Agent Toolkit Error:', error.response.status, error.response.data);
        throw new Error(`NeMo Agent Toolkit request failed with status ${error.response.status}: ${JSON.stringify(error.response.data)}`);
      } else if (error.request) {
        console.error('No response received from NeMo Agent Toolkit');
        throw new Error('No response received from NeMo Agent Toolkit');
      } else {
        throw new Error(`Error setting up NeMo Agent Toolkit request: ${error.message}`);
      }
    }
  }
...
const natClient = new NATClient();
export default natClient;